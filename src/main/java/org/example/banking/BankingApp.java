...
import org.example.banking.MongoDBConnection;
...
public class BankingApp {
    public static void main(String[] args) {
        ...
        HashMap<String, bank> accounts = loadAccounts(accountsCollection);
    ...
    private static HashMap<String, bank> loadAccounts(MongoCollection<Document> collection) {
        HashMap<String, bank> accounts = new HashMap<>();
        ...
        for (Document doc : docs) {
            ...
            double balance = (balObj instanceof Integer)
                    ...
                    : (Double) balObj;
            accounts.put(accNo, new bank(accNo, holder, balance));
        }
        return accounts;
    }

    private static void createAccount(Scanner scanner, HashMap<String, bank> accounts, MongoCollection<Document> collection) {
        ...
        bank newAcc = new bank(accNo, name, 0.0);
    ...
    private static void deposit(Scanner scanner, HashMap<String, bank> accounts, MongoCollection<Document> collection) {
    ...
    private static void withdraw(Scanner scanner, HashMap<String, bank> accounts, MongoCollection<Document> collection) {
        System.out.print("Enter account number: ");
        String accNo = scanner.nextLine();
        if (!accounts.containsKey(accNo)) {
            System.out.println("❌ Account not found.");
    ...
    private static void checkBalance(Scanner scanner, HashMap<String, bank> accounts) {
        System.out.print("Enter account number: ");
        String accNo = scanner.nextLine();
        if (!accounts.containsKey(accNo)) {
            System.out.println("❌ Account not found.");
    ...
    private static void viewAll(HashMap<String, bank> accounts) {
        System.out.println("📄 All Accounts:");
        for (bank acc : accounts.values()) {
            acc.displayDetails();
        }
    }
}